...
const valueArchivos = ["formulario_convalidacion", "cedula_identidad", "record_academico_carrera_origen", "contenidos_analiticos"];
const nomArchivos = ['2. Formulario de Convalidacion', '3. Cedula de Identidad', '4. Record Academico de la Carrera y Universidad que viene', '5. Contenidos Analiticos'];
...
async function enviarTramite() {
    ...
    if (confirmed) {
        ...
        try {
            const dat = { 'colum': 'r_informe_convalidacion', 'param': 'correcto', 'nrotramite': datosrecividos.nrotramite };
...
async function downloadDocument() {
    loading.value = true
    try {
        const message = await handleDownload(datosrecividos.nrotramite, 'c_informe_convalidacion', 'informe_convalidacion', datosrecividos.flujo, 'convalidacion_02');
        alert(message);
    } finally {
        loading.value = false
    }
