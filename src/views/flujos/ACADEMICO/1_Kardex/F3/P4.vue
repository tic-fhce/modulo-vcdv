        ...
        <div style="width: 80%;">
            <div class="card">
                ...
                <ListaArchivos :valueArchivos="valueArchivos" :nomArchivos="nomArchivos"
                    :mostrarObservacionesProp="true" :tabla="'convalidacion_01'" />
                <br><br>
            </div>
            ...
            <div class="card">
                <div v-if="swob">
                    <h5>
                        ...
                        <span :style="{ color: 'red', textTransform: 'uppercase' }">{{ obs }}</span>
                    </h5>
                    <h6 :style="{ color: 'blue', textTransform: 'uppercase' }">Realice las correcciones al documento de
                        convalidacion</h6>
                </div>
                <h5>GENERAR EL INFORME DE CONVALIDACION</h5>
...
import { handleUpload, handleUrl, handleDownload } from './Components/driveServiceConvalidacion01'
...
const docDrive = ref('INFORME CONVALIDACION.docx')
const swob = datosrecividos.observaciones
const obs = ref("")

const documentos = [{ archivo: '1. Informe de convalidacion', url: urlDoc, tipo: 'informe_convalidacion' }]
...
onMounted(async () => {
    const dat = { 'nrotramite': datosrecividos.nrotramite, 'columna': 'c_informe_convalidacion' };
    try {
        const res = await convalidacion01Service.obtenerColumna(dat);
        if (res.data != '') {
...
onMounted(async () => {
    if (obs) {
        const dat = { 'nrotramite': datosrecividos.nrotramite, 'columna': 'r_informe_convalidacion' };
        try {
            const res = await convalidacion01Service.obtenerColumna(dat);
            obs.value = res.data
        } catch (error) {
...
async function uploadDocument() {
    loading.value = true
    try {
        const message = await handleUpload(docDrive.value, datosrecividos.nrotramite, 'c_informe_convalidacion');
        alert(message);
        uploadDone.value = true;
        if (uploadDone.value) {
            getDocumentUrl();
...
async function getDocumentUrl() {
    loading.value = true
    try {
        urlDoc.value = await handleUrl(datosrecividos.nrotramite, 'c_informe_convalidacion');
    } finally {
        loading.value = false
    }
}
...
async function downloadDocument() {
    loading.value = true
    try {
        const message = await handleDownload(datosrecividos.nrotramite, 'c_informe_convalidacion', 'informe_convalidacion', datosrecividos.flujo, 'convalidacion_01');
        alert(message);
    } finally {
        loading.value = false
    }
...
async function cargarDocumento(nombreDocumento) {
    try {
        ...
        const dat = { nombre: nombreDocumento, nrotramite: nt, tabla: 'convalidacion_01', flujo: datosrecividos.flujo };
