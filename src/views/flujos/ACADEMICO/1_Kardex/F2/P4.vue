...
import { handleUpload, handleUrl, handleDownload } from './Components/driveServiceMateriaExtra'
...
const documentos = [{ archivo: '1. Aval de Aprobacion', url: urlDoc, tipo: 'aval' }]
...
async function uploadDocument() {
    loading.value = true
    try {
        const message = await handleUpload(docDrive.value, datosrecividos.nrotramite, 'c_aval');
        alert(message);
        uploadDone.value = true;
        if (uploadDone.value) {
            getDocumentUrl();
...
async function getDocumentUrl() {
    loading.value = true
    try {
        urlDoc.value = await handleUrl(datosrecividos.nrotramite, 'c_aval');
    } finally {
        loading.value = false
    }
}
...
async function downloadDocument() {
    loading.value = true
    try {
        const message = await handleDownload(datosrecividos.nrotramite, 'c_aval', 'aval', datosrecividos.flujo, 'materia_extra');
        alert(message);
    } finally {
        loading.value = false
    }
...
async function cargarDocumento(nombreDocumento) {
    try {
        ...
        const dat = { nombre: nombreDocumento, nrotramite: nt, tabla: 'materia_extra', flujo: datosrecividos.flujo };
