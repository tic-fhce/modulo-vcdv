<template>
    ...
    <div class="layout-main-container">
        <div style="width: 80%;">
            ...
            <div class="card">
                <h5>GENERAR EL DOCUMENTO DE RESOLUCION Y FIRMARLO PARA ENVIARLO A LA SIGUIENTE UNIDAD</h5>
...
import { handleUpload, handleUrl, handleDownload } from './Components/driveServiceTribunal'
...
const docDrive = ref('resolucion aprobacion de perfil de grado.docx')

const documentos = [{ archivo: '1. Resolucion de tribunal de grado', url: urlDoc, tipo: 'resolucion_tribunal' }]

const nomArchivos = ref(['1. Proyecto de resolucion de tribunal de grado']);
const valueArchivos = ref(["proyecto_resolucion_tribunal"]);

onMounted(async () => {
    const dat = { 'nrotramite': datosrecividos.nrotramite, 'columna': 'c_resolucion_tribunal' };
    try {
...
async function uploadDocument() {
    loading.value = true
    try {
        const message = await handleUpload(docDrive.value, datosrecividos.nrotramite, 'c_resolucion_tribunal');
        alert(message);
        uploadDone.value = true;
        if (uploadDone.value) {
            getDocumentUrl();
...
async function getDocumentUrl() {
    loading.value = true
    try {
        urlDoc.value = await handleUrl(datosrecividos.nrotramite, 'c_resolucion_tribunal');
    } finally {
        loading.value = false
    }
}
...
async function downloadDocument() {
    loading.value = true
    try {
        const message = await handleDownload(datosrecividos.nrotramite, 'c_resolucion_tribunal', 'resolucion_tribunal', datosrecividos.flujo, 'designacion_tribunal');
        alert(message);
    } finally {
        loading.value = false
    }
...
async function cargarDocumento(nombreDocumento) {
    try {
        ...
        const dat = { nombre: nombreDocumento, nrotramite: nt, tabla: 'designacion_tribunal', flujo: datosrecividos.flujo };
