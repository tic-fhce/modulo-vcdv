        ...
        <div style="width: 80%;">
            <div class="card">
                ...
                <ListaArchivos ref="valRef" :valueArchivos="valueArchivos" :nomArchivos="nomArchivos"
                    :mostrarObservacionesProp="true" :tabla="'aprobacion_perfil'"/>
                <br><br>
...
const nomArchivos = ['1. Nota dirigida al Director', '2. Nota de aceptacion del tutor', '3. Fotocopia simple de conclusion de estudios', '4. Record academico', '5. Perfil de grado'];
const valueArchivos = ["nota_director", "nota_tutor", "conclusion_estudios", "record_academico", "perfil_grado"];
...
onMounted(async () => {
    ...
    try {
        ...
        if (modalidad.value.data != 'Tesis') {
            nomArchivos.push('6. Carta de aceptacion de propuesta por la Institucion o empresa')
            valueArchivos.push('carta_institucion')
        }
    } catch (error) {
        ...
        if (confirmed) {
            const enviarSolicitud = async (index) => {
                if (index < imagenesSeleccionadas.value.length) {
                    ...
                    const env = { 'flujo': datosrecividos.flujo, 'proceso': datosrecividos.proceso, 'tramiteId': datosrecividos.nrotramite, 'comentario': 'correccion de datos del estudiante', 'condicion': '' };
