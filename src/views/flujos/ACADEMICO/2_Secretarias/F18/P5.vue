...
const documentos = [{ archivo: '1. Resolucion de perfil de grado', url: urlDoc, tipo: 'resolucion_perfil' }]

const nomArchivos = ref(['1. Proyecto de resolucion de perfil de grado']);
const valueArchivos = ref(["proyecto_resolucion_perfil"]);
...
onMounted(async () => {
    const dat = { 'nrotramite': datosrecividos.nrotramite, 'columna': 'c_resolucion_perfil' };
    try {
...
async function uploadDocument() {
    loading.value = true
    try {
        const message = await handleUpload('resolucion aprobacion de perfil de grado.docx', datosrecividos.nrotramite, 'c_resolucion_perfil');
        alert(message);
        uploadDone.value = true;
        if (uploadDone.value) {
            getDocumentUrl();
...
async function getDocumentUrl() {
    loading.value = true
    try {
        urlDoc.value = await handleUrl(datosrecividos.nrotramite, 'c_resolucion_perfil');
    } finally {
        loading.value = false
    }
}
...
async function downloadDocument() {
    loading.value = true
    try {
        const message = await handleDownload(datosrecividos.nrotramite, 'c_resolucion_perfil', 'resolucion_perfil', datosrecividos.flujo, 'aprobacion_perfil');
        alert(message);
    } finally {
        loading.value = false
    }
