...
const docDrive = ref('proyecto resolucion aprobacion de perfil de grado.docx')

const documentos = [{ archivo: '1. Proyecto de resolucion de tribunal de grado', url: urlDoc, tipo: 'proyecto_resolucion_tribunal' }]
...
onMounted(async () => {
    const dat = { 'nrotramite': datosrecividos.nrotramite, 'columna': 'c_proyecto_resolucion_tribunal' };
    try {
...
async function uploadDocument() {
    loading.value = true
    try {
        const message = await handleUpload(docDrive.value, datosrecividos.nrotramite, 'c_proyecto_resolucion_tribunal');
        alert(message);
        uploadDone.value = true;
        if (uploadDone.value) {
            getDocumentUrl();
...
async function getDocumentUrl() {
    loading.value = true
    try {
        urlDoc.value = await handleUrl(datosrecividos.nrotramite, 'c_proyecto_resolucion_tribunal');
    } finally {
        loading.value = false
    }
}
...
async function downloadDocument() {
    loading.value = true
    try {
        const message = await handleDownload(datosrecividos.nrotramite, 'c_proyecto_resolucion_tribunal', 'proyecto_resolucion_tribunal', datosrecividos.flujo, 'designacion_tribunal');
        alert(message);
    } finally {
        loading.value = false
    }
