...
import { handleUpload, handleUrl, handleDownload } from './driveServiceConvDocContratados';
import documentService from '@/services/document.service';
import convDocContratadosService from '@/services/convDocContratados.service';
...
const tablaBD = "conv_doc_contratados";
...
onMounted(async () => {
    ...
    try {
        for (const doc of localDocumentos.value) {
            ...
            const res = await convDocContratadosService.obtenerColumna(dat);
            if (res.data != '') {
                upC++;
            }
        }
...
async function uploadDocuments() {
    ...
    if (swCrearConv.value) {
        try{
            let datos = { 'nrotramite': datosrecividos.nrotramite }
            await convDocContratadosService.crearConvocatoria(datos);
        } catch {
            return null;
        }
    }
...
async function downloadDocument(data) {
    ...
    try {
        ...
        await convDocContratadosService.actulizarColumna(datAct);
