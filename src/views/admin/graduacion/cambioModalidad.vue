<template>
    <div class="card">
        <h4 style="color: blue; text-align: center;">CAMBIOS DE MODALIDAD DE GRADUACION</h4><br><br>
        ...
        <DataTable v-model:filters="filters" :value="Lista" paginator :rows="10" dataKey="id" filterDisplay="row"
            :loading="loading"
            :globalFilterFields="['tramite.id', 'tramite.estado', 'fechaFormateada', 'cambio', 'n_modalidad', 'n_titulo', 'n_tutor', 'nombres', 'apellidos', 'ci']">
            <template #header>
            ...
            <Column field="cambio" header="Cambio" :sortable="true" style="min-width:10%"></Column>
            <Column field="n_modalidad" header="Nueva Modalidad" :sortable="true" style="min-width:10%"></Column>
            <Column field="n_titulo" header="Nuevo Titulo" :sortable="true" style="min-width:10%"></Column>
            <Column field="n_tutor" header="Nuevo Tutor" :sortable="true" style="min-width:10%"></Column>
...
import { cambioModalidades } from '../js/cambioModalidad';
...
function listarTramites() {
    ...
    cambioModalidades.getData(selectedCarrera.value, selectedYear.value)
        .then(data => {
            //Lista.value = data;
...
async function cargarArchivo(data) {
    const dat = { 'nombre': data.documento, 'nrotramite': data.nrotramite, 'tabla': 'cambio_modalidad   ', 'flujo': data.flujo };
...
function documentoRequiereFirma(nombre) {
    const documentosConFirma = ['proyecto_resolucion_perfil', 'resolucion_perfil', 'solicitud_aprobacion_perfil'];
