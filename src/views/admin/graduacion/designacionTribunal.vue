<template>
    <div class="card">
        <h4 style="color: blue; text-align: center;">DESIGNACIONES DE TRIBUNAL DE GRADO</h4><br><br>
        ...
        <div class="flex justify-content-center mb-4 gap-3">
            ...
            <Button label="Generar Reporte" icon="pi pi-file-pdf" class="p-button-warning" @click="generatePDF"
                :disabled="!Lista.length" />
        </div>

        <DataTable v-model:filters="filters" :value="Lista" paginator :rows="10" dataKey="id" filterDisplay="row"
            :loading="loading"
            :globalFilterFields="['tramite.id', 'tramite.estado', 'fechaFormateada', 'aprobacion_Perfil.modalidad', 'aprobacion_Perfil.titulo', 'aprobacion_Perfil.tutor', 'nombres', 'apellidos', 'ci']">
            <template #header>
            ...
            <Column field="aprobacion_Perfil.modalidad" header="Modalidad" :sortable="true" style="min-width:10%">
            </Column>
            <Column field="aprobacion_Perfil.titulo" header="Titulo" :sortable="true" style="min-width:10%"></Column>
            <Column field="aprobacion_Perfil.tutor" header="Tutor" :sortable="true" style="min-width:10%"></Column>
            ...
            <Column header="Documentos" style="min-width:10%">
                <template #body="{ data }">
                    <div class="flex justify-content-left flex-wrap gap-3">
                        <Button @click="abrirModal(data)" label="Ver" severity="info" raised><i
...
import { designacionTribunals } from '../js/designacionTribunal';
...
const carreras = [
    ...
    { value: 'CN', label: 'Cine' },
    { value: 'FIL', label: 'Filosofica' },
    { value: 'HIS', label: 'Historia' },
    { value: 'LIN', label: 'Linguistica' },
...
function listarTramites() {
    ...
    designacionTribunals.getData(selectedCarrera.value, selectedYear.value)
        .then(data => {
...
async function cargarArchivo(data) {
    const dat = { 'nombre': data.documento, 'nrotramite': data.nrotramite, 'tabla': 'designacion_tribunal', 'flujo': data.flujo };
...
function documentoRequiereFirma(nombre) {
    const documentosConFirma = ['proyecto_resolucion_tribunal', 'resolucion_tribunal', 'solicitud_tribunal'];
...
function generatePDF() {
    ...
    doc.setFontSize(12);
    doc.text('Reporte de Designaciones de Tribunal de Grado', 12, y + imgHeight + 10);
    ...
    Lista.value.forEach((item) => {
        const rowData = [
            ...
            item.aprobacion_Perfil.modalidad,
            item.aprobacion_Perfil.titulo,
            item.aprobacion_Perfil.tutor,
            item.nombres,
            item.apellidos,
            item.ci,
        ];
    ...
    // Save the PDF
    doc.save('reporte_designaciones_tribunal.pdf');
}
</script>
